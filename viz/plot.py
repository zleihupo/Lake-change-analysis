...
if MODE == "drive":
    from google.colab import drive
    drive.mount('/content/drive')
    # Define file paths (adjust according to your directory structure)
    ...
    # HTML maps (fuzzy match, no need to change)
    ...
    # Time series & importance CSVs (adjust if needed)
    ...
    print("👉 Please select files (multiple allowed): test images/masks ZIP, NPY, HTML, CSV etc.")
...
def load_pairs(img_dir, msk_dir):
    ...
    if not pairs: raise RuntimeError("No (image, mask) pairs found, check directories.")
    return pairs
...
def save_triptych(img_path, msk_path, out_path, size=(256, 256), show=True):
    ...
    # Read mask and optionally resize; convert to binary (0/1), keep edges crisp
    msk = read_mask_bin(msk_path, size=size)

    # 1 row × 3 cols: Image / GSW mask / overlay
...
# Select first/middle/last 3 samples
...
# ===== Fig.8 (robust version): automatically compute area_index and plot small multiples by region =====
import os, numpy as np, pandas as pd, matplotlib.pyplot as plt, math

# If auto-search fails, manually set CSV path:
...
if "year" not in df.columns:
    if "date" in df.columns:
        ...
        raise ValueError("CSV must contain year or date column (cannot infer).")

# 2) Infer lake / region columns
def find_col(cands):
    for c in cands:
...
lake_col   = find_col(["lake","lakename","name"])
region_col = find_col(["region","zone","area"])
if lake_col is None:
    print("⚠️ No lake column detected, using global baseline.")
if region_col is None:
    print("⚠️ No region column detected, assigning all to 'All'.")
    region_col = "region"
    df[region_col] = "All"

# 3) Find area column
area_candidates = ["area_index","area_m2","area_km2","area"]
area_col = None
...
if area_col is None:
    for col in df.columns:
        if "area" in col.lower():
            area_col = col; break
if area_col is None:
    raise ValueError("No area column found in CSV (expected area_m2/area_km2/area).")

print(f"Detected columns: year=year, region={region_col}, lake={lake_col}, area={area_col}")

# 4) If area_index not provided, compute it
...
# 5) Aggregate by region-year and plot
