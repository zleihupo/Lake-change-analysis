...
function getAnnualSummerClimate(lake) {
  ...
  return ee.FeatureCollection(years.map(function(y){
    ...
    var pet = climateMean.select('potential_evaporation_sum').reduceRegion({
      reducer: ee.Reducer.mean(), geometry: geom, scale: 9000, maxPixels: 1e13
    }).get('potential_evaporation_sum');

    var snow_depth = climateMean.select('snow_depth').reduceRegion({
      reducer: ee.Reducer.mean(), geometry: geom, scale: 9000, maxPixels: 1e13
    }).get('snow_depth');

    var snow_cover = climateMean.select('snow_cover').reduceRegion({
      reducer: ee.Reducer.mean(), geometry: geom, scale: 9000, maxPixels: 1e13
    }).get('snow_cover');

    var temp_K = climateMean.select('temperature_2m').reduceRegion({
      reducer: ee.Reducer.mean(), geometry: geom, scale: 9000, maxPixels: 1e13
    }).get('temperature_2m');

    var temp_C = ee.Number(temp_K).subtract(273.15); // Kelvin → Celsius
    ...
    return ee.Feature(null, {
      ...
      pet_mm: ee.Number(pet).multiply(1000),              // m → mm
      snow_depth_cm: ee.Number(snow_depth).multiply(100), // m → cm
      snow_cover_pct: ee.Number(snow_cover).multiply(100) // fraction → %
    });
