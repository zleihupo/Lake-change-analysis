...
function monthSeq(start, end) {
  ...
  var n = e.difference(s, 'month').int();
  return ee.List.sequence(0, n.subtract(1)).map(function(k){
    var d1 = s.advance(k, 'month');
    var d2 = d1.advance(1, 'month');
    return ee.Dictionary({start: d1, end: d2, y: d1.get('year'), m: d1.get('month')});
  });
}
...
function getSummerClimate(lake) {
  ...
  var feats = ee.FeatureCollection(months.map(function(d){
    d = ee.Dictionary(d);
    var d1 = ee.Date(d.get('start'));
    var d2 = ee.Date(d.get('end'));
    ...
    return ee.Algorithms.If(isSummer, (function(){
      ...
      var pet_mm = ee.Number(
        img.select('potential_evaporation')
          .reduceRegion({reducer: ee.Reducer.mean(), geometry: geom, scale: 9000, maxPixels: 1e13})
          .get('potential_evaporation')
      ).multiply(1000); // m → mm

      var snow_depth_cm = ee.Number(
        img.select('snow_depth')
          .reduceRegion({reducer: ee.Reducer.mean(), geometry: geom, scale: 9000, maxPixels: 1e13})
          .get('snow_depth')
      ).multiply(100); // m → cm

      var snow_cover_pct = ee.Number(
        img.select('snow_cover')
          .reduceRegion({reducer: ee.Reducer.mean(), geometry: geom, scale: 9000, maxPixels: 1e13})
          .get('snow_cover')
      ).multiply(100); // fraction → %

      return ee.Feature(null, {
        lake: name,
        hemisphere: hemi,
        date: d1.format('YYYY-MM'),
        year: y,
        month: m,
        precip_mm: precip_mm,
        pet_mm: pet_mm,
        snow_depth_cm: snow_depth_cm,
        snow_cover_pct: snow_cover_pct
      });
    })(), null);
