...
lakes.forEach(function(lake) {
  ...
  var areaGSW = ee.ImageCollection("JRC/GSW1_4/MonthlyHistory")
    ...
    .filter(ee.Filter.inList('month', summer_months))
    .map(function(img) {
...
var ndwiList = years.map(function(year) {
...
var areaNDWI = ee.FeatureCollection(ndwiList).filter(ee.Filter.notNull(['area_m2']));
  ...
  var tempTS = ee.ImageCollection("ECMWF/ERA5_LAND/MONTHLY")
    ...
    .map(function(img) {
      ...
      return ee.Feature(null, {
        date: img.date().format('YYYY-MM'),
        temp_C: ee.Number(temp.get('temperature_2m')).subtract(273.15)
      });
    });
  ...
  allFeatures = allFeatures.concat(ee.Algorithms.If(gswSize.gt(0), joinedGSW.toList(gswSize), ee.List([])));
  allFeatures = allFeatures.concat(ee.Algorithms.If(ndwiSize.gt(0), joinedNDWI.toList(ndwiSize), ee.List([])));
});
