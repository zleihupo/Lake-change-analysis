...
lakes.forEach(function(lake) {
  ...
  var monthlyList = years.map(function(year) {
    return ee.List(summer_months).map(function(month) {
      ...
      var s2 = ee.ImageCollection("COPERNICUS/S2_SR_HARMONIZED")
        ...
        .map(function(img) {
          return img.select(['B2','B3','B4','B8'], ['blue','green','red','nir']);
        })
      ...
      var ls_all = ee.ImageCollection("LANDSAT/LE07/C02/T1_L2")
        ...
        .map(function(img) {
          ...
          var blueBand = ee.Algorithms.If(spacecraft.compareTo('LANDSAT_7').eq(0), 'SR_B1', 'SR_B2');
          var greenBand = ee.Algorithms.If(spacecraft.compareTo('LANDSAT_7').eq(0), 'SR_B2', 'SR_B3');
          var redBand = ee.Algorithms.If(spacecraft.compareTo('LANDSAT_7').eq(0), 'SR_B3', 'SR_B4');
          var nirBand = ee.Algorithms.If(spacecraft.compareTo('LANDSAT_7').eq(0), 'SR_B4', 'SR_B5');
          return scaled.select(
            [blueBand, greenBand, redBand, nirBand],
            ['blue', 'green', 'red', 'nir']
      ...
      var finalFeat = ee.Algorithms.If(
        ...
        ee.Algorithms.If(lsimg,
          calcAreaNDWI(lsimg, geom, name, 'Landsat-7/8/9'),
          null
        ),
        ee.Algorithms.If(
          ee.Number(ee.Feature(sentinelFeat).get('area_m2')).gt(0),
          sentinelFeat,
          ee.Algorithms.If(lsimg,
            calcAreaNDWI(lsimg, geom, name, 'Landsat-7/8/9'),
            sentinelFeat
  ...
  var fc = ee.FeatureCollection(monthlyList).filter(ee.Filter.notNull(['area_m2']));
  allFeatures = allFeatures.concat(fc.toList(fc.size()));
});
